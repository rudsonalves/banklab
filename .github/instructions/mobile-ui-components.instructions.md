---
description: "Use when creating or editing shared UI components files: SafeScaffold, AppSnackbar, BasicTextFormField, CpfInputFormatter, MaterialTheme, input formatters, layout shells, or theme helpers. Covers promotion rules, widget design, feedback helpers, and dependency constraints for ui/components."
applyTo: "mobile/lib/ui/components/**"
---
# Shared UI Components Agent Guide

This guide applies to reusable widgets, input elements, feedback helpers,
formatters, and theme primitives under `mobile/lib/ui/components`.

Use this area for UI elements with real potential to be reused across multiple
screens. Keep page-specific widgets under `mobile/lib/ui/pages/.../widgets`
until reuse is clear.

This folder is also the staging area for a possible future internal mobile
widget/feature package. Code promoted here should be stable enough to represent
shared app UI behavior, not just extracted to make a page file shorter.
...
`ui/components` belongs to the presentation layer. It provides shared visual and
...
Put code in `ui/components` when it is:
...
Do not put these in `ui/components`:
...
Before moving a widget from a page into `ui/components`, check:
...
Avoid feature names in `ui/components` unless the component is truly part of the
