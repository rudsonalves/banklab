---
description: "Use when creating or editing use case files under domain/usecases. Covers when to create a use case, multi-repository orchestration, Result/AsyncResult return, injection into view models, and rules for keeping use cases widget-free."
applyTo: "mobile/lib/domain/usecases/**"
---
...
- orchestration is not a UI concern but also does not belong inside one
  repository
- the UI needs a small app-facing input type, such as `TransferDraft`, to avoid
  constructing a transport DTO directly in the page
